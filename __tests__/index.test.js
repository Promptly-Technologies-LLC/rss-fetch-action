...
describe('fetchRssFeed function', () => {
  describe('Validation tests', () => {
    beforeEach(() => {
      // Mock the extract function using dynamic import
      jest.doMock('@extractus/feed-extractor', () => ({
        __esModule: true,
        extract: jest.fn(),
      }));
    });
    ...
    it('should set failed if feed URL is not provided', async () => {
      ...
      expect(core.setFailed).toHaveBeenCalledWith('Feed URL is not provided or invalid');
    });
    ...
    it('should set failed if file extension is not .json', async () => {
      ...
      // Expect that core.setFailed was called with the expected error message
      expect(core.setFailed).toHaveBeenCalledWith('File path extension must be .json');
    });
  });
  ...
  describe('Input Parsing', () => {
    ...
    it('should set failed if parserOptions is invalid JSON', async () => {
      ...
      // Expect that core.setFailed was called with the expected error message
      expect(core.setFailed).toHaveBeenCalledWith("Failed to parse parserOptions input: Unexpected token 'i', \"invalid JSON\" is not valid JSON");
    });
    ...
    it('should set failed if fetchOptions is invalid JSON', async () => {
      ...
      // Expect that core.setFailed was called with the expected error message
      expect(core.setFailed).toHaveBeenCalledWith("Failed to parse fetchOptions input: Unexpected token 'i', \"invalid JSON\" is not valid JSON");
    });
    ...
    it('should evaluate function in parserOptions if provided as string', async () => {
      ...
      process.env.INPUT_PARSER_OPTIONS = JSON.stringify({
        getExtraEntryFields: "function(feedEntry) { return { 'content:encoded': feedEntry['content:encoded'] || '' }; }"
      });
  ...
  describe('extract function interaction', () => {
    ...
    it('should handle errors thrown by extract function', async () => {
      ...
      // Expect that core.setFailed was called with the expected error message
      expect(jest.isMockFunction(extract)).toBe(true);
      expect(core.setFailed).toHaveBeenCalledWith('Failed to fetch or parse feed: Some error');
    });
  });
  ...
  describe('fetchRssFeed function', () => {
    describe('File operations', () => {
      beforeEach(async () => {
        ...
        jest.doMock('@extractus/feed-extractor', () => ({
          __esModule: true,
          extract: jest.fn().mockResolvedValue({ title: 'Test Feed' }),
        }));
        const feedExtractor = require('@extractus/feed-extractor');
        extract = feedExtractor.extract;
      });
      ...
      it('should create directory if it does not exist', async () => {
        ...
        // Verify
        expect(fs.mkdirSync).toHaveBeenCalledWith(path.dirname(process.env.INPUT_FILE_PATH), { recursive: true });
      });
  ...
  describe('Success logging', () => {
    ...
    it('should log success message if everything goes well', async () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(`RSS feed saved to ${process.env.INPUT_FILE_PATH} successfully!`);
    });
  });
