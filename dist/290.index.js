...
const XMLParser = __webpack_require__(2380);
const XMLBuilder = __webpack_require__(660);
...
const buildFromOrderedJs = __webpack_require__(2462);
...
const util = __webpack_require__(8280);
const xmlNode = __webpack_require__(7462);
const readDocType = __webpack_require__(6072);
const toNumber = __webpack_require__(4526);
...
const { buildOptions} = __webpack_require__(6993);
const OrderedObjParser = __webpack_require__(5832);
const { prettify} = __webpack_require__(2882);
const validator = __webpack_require__(1739);
...
var named_references_1 = __webpack_require__(6068);
var numeric_unicode_map_1 = __webpack_require__(5439);
...
var node_ponyfill = __webpack_require__(9805);
...
const profetch = async (url, options = {}) => {
  ...
  const res = await node_ponyfill(target + encodeURIComponent(url), {
    headers,
    signal,
  })
  return res
...
/* harmony default export */ const retrieve = (async (url, options = {}) => {
  ...
  const res = proxy ? await profetch(url, { proxy, signal }) : await node_ponyfill(url, { headers, agent, signal })
...
;// CONCATENATED MODULE: ./node_modules/bellajs/src/utils/detection.js
// utils / detection
...
const isObject = (val) => {
  return ob2Str(val) === '[object Object]' && !detection_isArray(val)
}
...
const isEmpty = (val) => {
  return !val || isNil(val) ||
    ...
    (detection_isArray(val) && val.length === 0) ||
    (isObject(val) && Object.keys(val).length === 0)
}

const detection_hasProperty = (ob, k) => {
  if (!ob || !k) {
    return false
  }
...
var fxp = __webpack_require__(2603);
...
const validate = (xml) => {
  return (!detection_isString(xml) || !xml.length) ? false : fxp.XMLValidator.validate(xml) === true
}
...
;// CONCATENATED MODULE: ./node_modules/bellajs/src/utils/string.js
// utils / string



const string_toString = (input) => {
  const s = detection_isNumber(input) ? String(input) : input
  if (!detection_isString(s)) {
...
const replaceAll = (s, alpha, beta) => {
  let x = string_toString(s)
...
var lib = __webpack_require__(2589);
...
const getText = (val) => {
  ...
  return txt ? (0,lib.decode)(String(txt).trim()) : ''
}
      ...
      : isObject(val) && detection_hasProperty(val, '@_href')
        ...
        : isObject(val) && detection_hasProperty(val, '@_url')
          ...
          : isObject(val) && detection_hasProperty(val, '_attributes')
            ...
            : detection_isArray(val) ? getEntryLink(val) : ''
...
const getOptionalTags = (val, key) => {
  ...
  if (key === 'category') {
    return detection_isArray(val) ? val.map(getCategory) : getCategory(val)
  }
  if (key === 'enclosure') {
...
const parseJson = (data, options) => {
  ...
  const items = detection_isArray(item) ? item : [item]
...
const flatten = (feed, baseUrl) => {
  ...
  const output = {
    ...
    item: detection_isArray(item) ? entries : entries[0],
  }
  return output
}
...
const parseAtomFeed_flatten = (feed, baseUrl) => {
  ...
  const entries = detection_isArray(entry) ? entry : [entry]
  ...
  const output = {
    ...
    title: getText(title),
    link: getPureUrl(link, id, baseUrl),
    entry: detection_isArray(entry) ? items : items[0],
  }
  return output
}
