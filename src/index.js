...
async function fetchRssFeed() {
  try {
    ...
    try {
      ...
      throw new Error(`Failed to parse parserOptions input: ${jsonError.message}`);
    }

    try {
      fetchOptions = JSON.parse(process.env.INPUT_FETCH_OPTIONS || '{}');
    } catch (jsonError) {
      throw new Error(`Failed to parse fetchOptions input: ${jsonError.message}`);
    }

    // If parserOptions contains a function as a string, eval it (Be cautious with eval)
    if (parserOptions.getExtraEntryFields && typeof parserOptions.getExtraEntryFields === 'string') {
      parserOptions.getExtraEntryFields = eval(`(${parserOptions.getExtraEntryFields})`);
    }
